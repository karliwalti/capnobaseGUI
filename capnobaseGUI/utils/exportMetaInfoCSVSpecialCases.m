...
foldercontent=dir([folder '*mat']); %'*_meta.mat'
...
for k=1:length( handles.data.name)%179%
    ...
    switch type
        case 'meta'
            ...
            value{signalcounter,counter+1}=[(handles.data.name{k}(1:end-5)) '.png'];%upper
            header{counter+2}='realID';
            value{signalcounter,counter+2}=[(handles.data.name{k}(1:end-5)) ''];%upper
            header{counter+3}='subjectID';
            value{signalcounter,counter+3}=handles.data.name{k}(1:end-7);
            header{counter+4}='ratingNumID';
            value{signalcounter,counter+4}=k+100;
            header{counter+5}='sensorID';
            value{signalcounter,counter+5}=1;%str2num(handles.data.name{k}(1:4))+1000;
            header{counter+6}='treatmentID';
            %            id=str2num(handles.data.name{k}(1:4))+2000;
            %             idx=find(treatment.data(:,2)==id,1,'first');
            value{signalcounter,counter+6}=1;%treatment.data(idx,1);
            header{counter+7}='subID';
            %          id=str2num(handles.data.name{k}(1:4));
            %         idx=find(subjects.data(:,2)==id,1,'first');
            value{signalcounter,counter+7}=1;%subjects.data(idx,1);
            header{counter+8}='ratingID';
            value{signalcounter,counter+8}=1;%signalcounter+200;%[handles.data.name{k}(1:6) ''];
